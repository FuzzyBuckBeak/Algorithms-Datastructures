...
class AdjacencyMatrix<T: Hashable>: Graph {
    ...
    func bfsRecursive(source: Vertex<T>) -> [Vertex<T>] {
        var queue: [Vertex<T>?] = []
        ...
        bfsHelper(queue: &queue, enqueued: &enqueue, visited: &visited)
        return visited
    }
    
    private func bfsHelper(queue: inout [Vertex<T>?], enqueued: inout Set<Vertex<T>>, visited: inout [Vertex<T>]) {
        guard let vertex = queue.removeFirst() else { return }
        ...
        bfsHelper(queue: &queue, enqueued: &enqueued, visited: &visited)
    }
}
